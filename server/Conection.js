...
app.get("/api/personalSalud", (req, res) => {
  ...
  let query = `
    ...
    WHERE p.estado = 1 AND ps.rol = 'medico'
  `;
...
app.get("/api/criterios", (req, res) => {
  const query =
    "SELECT idCriterioIngreso, tipo, subtipo, estadoIngreso FROM tuberculosisproyect.criterioingreso WHERE estado = 1;";
...
app.post("/api/pacientes", (req, res) => {
  ...
  const query =
    "INSERT INTO persona (nombres, primerApellido, segundoApellido, numeroCelular, CI, direccion, sexo, fechaNacimiento, estado, EstablecimientoSalud_idEstablecimientoSalud, idCriterioIngreso) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?)";
